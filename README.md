...
| [BCE — blog](src/sources/rss.ts) | 1 | non | **Billet signé, pas une décision** : n'engage que ses auteurs. Pour un chiffre ou une décision, remonter au communiqué (EP-001) |
| [Business Recorder](src/sources/rss.ts) | **3** | non | Flux **doublement encodé** : `&lt;p&gt;&lt;strong&gt;` — le balisage repartait en clair vers l'Analyste. Les balises sont retirées **deux fois**, avant et après décodage |
| [arXiv](src/sources/rss.ts) | **3** | non | **Prépublication : personne n'a relu.** Tier 3 malgré l'institution — le tier dit ce que vaut le *contenu* pour fonder un fait. Débit imposé par ses conditions d'usage : une requête toutes les 3 s, aucun 429 ne viendrait avertir |
| [GitHub — versions](src/sources/rss.ts) | **2** *(par chemin)* | non | Le numéro et la date se vérifient à l'adresse ; **tout ce que la note affirme** est déclaré par l'éditeur. Seuls trois chemins mesurés sont au tier 2 — `github.com` héberge n'importe qui. Trois dépôts écartés : marqueurs d'intégration continue, numéros de compilation, *release candidates* |
| [Hugging Face](src/sources/huggingface.ts) | **2** *(par chemin)* | non | **Deux chiffres, un facteur quinze** : `downloads` = 30 jours, `downloadsAllTime` = cumul, et le second n'est pas rendu sans le demander. Un « téléchargement » compte chaque requête HTTP vers un fichier de configuration, CI comprise, **sans déduplication** : ce ne sont pas des utilisateurs (EP-005) |
...
**Sur la technologie — un domaine qui produit plus d'annonces que de faits.** Les trois émetteurs branchés le 19 septembre 2026 documentent trois moments distincts : ce qui est **déposé** (arXiv), ce qui est **livré** (versions GitHub), ce qui est **effectivement repris** (diffusion mesurée sur le Hub). Aucun n'est branché sur la seule foi d'un HTTP 200 : six dépôts GitHub ont été mesurés, trois écartés ; `sort=createdAt` sur le Hub ne rend que des dépôts personnels à zéro téléchargement, et le blog du Hub publie des notes d'ingénierie, pas des annonces. Les motifs sont **datés et conservés dans le code**, faute de quoi quelqu'un rebranchera un flux en constatant qu'il répond.

Deux pièges sont tenus par du code, pas par une consigne :

- **Une performance annoncée par son éditeur n'est pas un fait.** Une note de version est au tier 2 à juste titre — la publication est un fait à distance zéro — mais le même document affirme « deux fois plus rapide », et `FACT_NEEDS_PRIMARY_SOURCE` ne voit rien puisque le tier 2 est là. Le tier mesure la distance à la donnée, pas l'indépendance de qui la produit : `VENDOR_CLAIMED_PERFORMANCE` refuse, et **une** source indépendante suffit à lever la règle. Le protocole ne fait pas taire, il requalifie.
- **Un téléchargement n'est pas un utilisateur.** Le chiffre voyage avec sa définition, et un modèle dont le cumul manque est **écarté** plutôt que servi avec sa fenêtre de trente jours à la place.
